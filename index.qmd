...
of children with both typical speech development as well as children with speech and motor
...
@tocs2007]. Example items include *cowboy boots* and *put all
the toys away*. These speech samples are used to calculate articulation rate,
...
and we notate IPA using conventional forward slashes as in /t/ or /u/.
The IPA includes many non-English-alphabet characters or diacritics which can
cause headaches for computing systems. A more computing-friendly
phonetic alphabet is [ARPABET](https://en.wikipedia.org/wiki/ARPABET) which uses
ASCII characters, specifically all-caps English-alphabet letters. We use a
...
the MFA alignments. We note here that the alignments performed in
...
### Praat shortcuts {#praat-shortcuts}
  ...
  - Select next interval/tier – <kbd>Alt-arrows</kbd>

  - Remove boundary – click on or after a boundary, and type <kbd>Alt-Backspace</kbd> (do this for each tier that a boundary needs to be
    deleted on).
