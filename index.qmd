...
# https://quarto.org/docs/authoring/front-matter.html#roles
  - name: Lucas Annear
    roles: Conceptualization of hand-correction project. Wrote guide for our group and curated examples.
  - name: Henry Nomeland
    roles: Converted guide to Markdown, refined examples. Continued development of guide.
  - name: Tristan Mahr
    roles: Conceptualization of hand-correction project. Converted guide to Quarto, revised guide for wider publication.
format:
  html:
...
<!--
notes: 
- audience is for our people // it's a protocol
- need an abstract
--> 
...
of children with both typical speech development as well as children
with speech and motor impairments due to cerebral palsy. In these
recordings the child is repeating words and phrases from the *Test of
Children’s Speech* [TOCS, @tocs2007]. Example items include *cowboy
boots* and *put all the toys away*. These speech samples are used to
calculate articulation rate, speech sound accuracy, and measures of how
intelligible a child is.



We use the Praat software to visualize and annotate the acoustic features of these speech 
samples. @fig-1 shows a screenshot for the sentence  
...
sound files directly. We need to create a separate file that stores
...
<!--
maybe a callout box or glossary
-->


It would be time-consuming to set down the word and phoneme boundaries for a 
speech sample by hand. A **forced aligner** is a software tool that 
automates the creation of word and phoneme boundaries. The aligner takes an 
audio file (typically a .wav file) and a transcript of the speech sample 
(usually a .txt or .lab file), and it uses a speech recognition model to 
create textgrid. It is called a *forced* aligner because it has to follow the 
transcript and set down intervals for each word and phoneme in the transcript. 
(It is not allowed to identify the words itself.)
...
We can open these textgrids along with the audio file in a program called **Praat**. When the textgrids and audio files are paired together in Praat, it is easier and quicker for us to make measurements because we have labels for each word and sound, as well as the time intervals during which these events occur.

<aside>*Praat* is Dutch for "talk" or "talking", and it sounds like “praht”
/pra:t/.</aside>.
...
Forced aligners are far from perfect. When a forced aligner creates a textgrid, the initial placement of boundaries for words and sounds may not be accurate. During the **hand-correction** process, researchers manually adjust the boundaries of textgrids that have been automatically created using the aligner. 

Our group uses the Montreal Forced Aligner (MFA) or Wav2TextGrid (W2TG) aligners. For this document, we assumed that original textgrids were created by MFA.

When MFA pairs a transcription with an audio file, it often produces
fairly accurate boundary placements. In these cases the boundaries may
only need some slight shifting, or even no adjustment at all. Sometimes,
the boundaries are placed at the wrong times, and in more severe cases, the
alignment goes "off the rails" with intervals being assigned to entirely
wrong words. These boundaries require hand correction. See example in @fig-3.



::: {#fig-3 layout-nrow="3"}
![*Before correction*: Boundaries for *cowboy* were placed too soon, and
the boundaries for *boots* include the *boy* part of *cowboy*. In the
lower phoneme tier, there is vowel in *boy* takes up negligible amount
of space; it is to narrow for Praat to print the
characters.](alignmentGuide/media/image6.png){width="70%"}
  
![*Comparison of word boundaries*: The black and gold shapes mark the intervals where *cowboy* and *boots* actually occur.)](alignmentGuide/media/image7.png){width="70%"}

![*After correction*.](alignmentGuide/media/image8.png){width="70%"}

Textgrid of *cowboy boots* before and after hand correction.
:::



The goal of the researcher performing hand correction is to ensure
proper boundary placement. This guide is intended to help visually
identify alignment issues by using spectrograms and waveforms, and it is
designed to be a guide for decision-making when boundary placement is
...
<!-- IPA -->
<!--   ~ International Phonetic Alphabet, the conventional way to represent speech  -->
<!--   sounds -->
...
the [International Phonetic Alphabet (IPA)][ipa], and we notate IPA
using conventional forward slashes as in /t/ or /u/. 
The IPA includes many non-English-alphabet characters and diacritics,
and these characters can raise problems in older plain-text computing
workflows.


The second phonetic alphabet is [ARPABET][arpabet] which uses
[ASCII][ascii] characters to represent speech sounds. (Think of the
ASCII characters as the basic characters one can write with an American
keyboard.) We use a particular flavor of ARPABET called CMUBET. The IPA
characters /ʃ/, /ŋ/, and /ə/, for example, are written as SH, NG and AH0 in
this phonetic alphabet. CMUBET is the alphabet used by the [CMU
pronunciation dictionary][cmu], and it is supported by MFA.

<aside>CMUBET is the ARPABET but with numbers added onto vowels indicate stress.
Because numbers mark stress, `AH0` is /ə/ but `AH1` or higher is /ʌ/. The same
goes for `ER` and /ɚ/ and /ɝ/.</aside>

We note here that the alignments performed in the WISC Lab and any
alignments using the CMU dictionary are based on North American
pronunciations. @tbl-vowels and @tbl-consonants show vowels and
consonants in North American English in the two alphabets.

[cmu]: http://www.speech.cs.cmu.edu/cgi-bin/cmudict "CMU pronunciation dictionary"

[ipa]: https://www.internationalphoneticassociation.org/content/full-ipa-chart "IPA chart"

[arpabet]: https://en.wikipedia.org/wiki/ARPABET "Wikipedia: ARPABET"

[ascii]: https://en.wikipedia.org/wiki/ASCII#Character_set "Wikipedia: ASCII"
...
This guide assumes that you have audio files and their corresponding
forced alignment textgrids (created by MFA). If you do not have
forced-alignment textgrids, see MFA's [installation
guide][mfa-install] and [first-steps
instructions][mfa-first-steps]. Eleanor Chodroff also has a [chapter
on MFA][echo-mfa] in her [corpus phonetics tutorial][echo-book],
although this guide may fall out of sync with MFA over time.

[mfa-install]: https://montreal-forced-aligner.readthedocs.io/en/latest/installation.html "MFA installation page"

[mfa-first-steps]: https://montreal-forced-aligner.readthedocs.io/en/latest/first_steps/index.html#first-steps-align-pretrained "MFA First Steps page"

[echo-book]: https://eleanorchodroff.com/tutorial/index.html "Corpus Phonetics Tutorial online book"

[echo-mfa]: https://www.eleanorchodroff.com/tutorial/montreal-forced-aligner.html#montreal-forced-aligner "MFA tutorial in the Corpus Phonetics Tutorial"

### Praat

### Download Praat {#download-praat}

Praat can be downloaded from <https://www.fon.hum.uva.nl/praat/>, and its 
website provides ample documentation about installation.

We often use Praat scripts in our lab. These are short programs that
control Praat as a user would but in an automated way. For example, we
can have Praat open pairs of .wav and .TextGrid files for hand
correction one at a time with a Praat script. These script files end in
`.praat`. 

When working with the Praat editor window, which we use when correcting
textgrids, the following keyboard shortcuts are helpful.

  - Zoom in – <kbd>Ctrl-I</kbd>

  - Zoom out – <kbd>Ctrl-O</kbd>

  - Zoom to selection – <kbd>Ctrl-N</kbd>

  - Select prev/next interval – <kbd>Alt-left arrow</kbd> and
    <kbd>Alt-right arrow</kbd>
  
  - Select prev/next tier – <kbd>Alt-up arrow</kbd> and <kbd>Alt-down
    arrow</kbd>

  - Remove boundary – click on or after a boundary and press
    <kbd>Alt-Backspace</kbd>. Remember that a boundary needs to be
    deleted on each tier that cointains it.
...
Although the remainder of this guide shows specific examples of where alignment boundaries should be placed in certain circumstances, there are a few overarching principles to keep in mind. These should be used in conjunction with the remainder of the guide to make more efficient decisions regarding boundary placement.

1.  **What counts as speech?**  \
    We're counting as speech any sound generated by the articulators
    that carries information about the sounds produced in the utterance.

2.  **When should a boundary be moved?**  \
    Boundaries should remain where they were placed by the forced
    aligner unless you have visual or auditory evidence that a given
    boundary does not line up with the beginning or end of a sound.
    Minimizing unnecessary movements promotes consistency across
    alignments.
